...
pub struct Render<T: 'static, M: 'static> {
    render: Option<Box<dyn Fn(&T, &::failure::Error) -> Html<PersistFailed<T, M>>>>,
}

impl<T, M: 'static> Render<T, M> {
    fn new<F: Fn(&T, &::failure::Error) -> Html<PersistFailed<T, M>> + 'static>(f: F) -> Self {
        Render {
...
impl<T: 'static, M: 'static> PersistFailed<T, M> {
    pub fn render<F: Fn(&T, &::failure::Error) -> Html<PersistFailed<T, M>> + 'static>(f: F) -> Render<T, M> {
        Render::new(f)
    }
}
