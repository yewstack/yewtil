...
pub struct Render<T: 'static, M: 'static> {
    render: Option<Box<dyn Fn(&T) -> Html<Fetched<T, M>>>>,
}

impl<T, M: 'static> Render<T, M> {
    fn new<F: Fn(&T) -> Html<Fetched<T, M>> + 'static>(f: F) -> Self {
        Render {
...
impl<T: 'static, M: 'static> Fetched<T, M> {
    pub fn render<F: Fn(&T) -> Html<Fetched<T, M>> + 'static>(f: F) -> Render<T, M> {
        Render::new(f)
    }
}

#[derive(Properties)]
pub struct FetchedProps<T: 'static, M: 'static> {
    children: Children<Fetched<T, M>>,
...
impl<T: 'static, M: 'static> Component for Fetched<T, M> {
    type Message = M;
    type Properties = FetchedProps<T, M>;
